 ...
 *  via the messageHandler `operation`. This file expects there to be a message handler function
 *  registered at `window.webkit.messageHandlers.operation.postMessage` for these messages.
...
function messageOperationError(operationId) {
    ...
    window.webkit.messageHandlers.operation.postMessage(JSON.stringify(message));
}
